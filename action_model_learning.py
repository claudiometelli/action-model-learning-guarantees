...
from pddl_plus_parser.exporters.domain_exporter import DomainExporter as PDDLExporter
...
def main() -> None:
    ...
    for action_name in domain.actions:
        lower_preconds[action_name] = utils.get_action_space(domain, action_name)
        upper_preconds[action_name] = [State(None, None)]
        lower_effects[action_name] = State(None, None)
        upper_effects[action_name] = utils.get_action_space(domain, action_name)
    ...
    for row in dataset:
        ...
        if action_result:
            lower_preconds[action_name] = utils.action_intersection(
                state,
            ...
            lower_effects[action_name] = utils.effect_union(
                state,
                next_state,
                domain.actions[action_name],
                action_params,
            ...
            upper_effects[action_name] = utils.action_intersection(
                next_state,
                domain.actions[action_name],
                action_params,
    ...
    for row in dataset:
        ...
        if not action_result:
            upper_preconds[action_name] = utils.update_preconds_ub(
                state,
                domain.actions[action_name],
                action_params,
                lower_preconds[action_name],
    ...
    for action_name in domain.actions:
        upper_preconds[action_name] = utils.reduce_upper_bound(upper_preconds[action_name])
    
    if DEBUG:
        for action_name in domain.actions:
            print(f"\n{'='*60}\nACTION ANALYSIS FOR ACTION: {action_name}\n{'='*60}")
            print(f"{'='*40}\n PRECONDITIONS LOWER BOUND: {action_name}\n{'='*40}\n{utils.state_to_str(lower_preconds[action_name])}")
            print(f"{'='*40}\n PRECONDITIONS UPPER BOUND: {action_name}\n{'='*40}")
            for hypotesis in upper_preconds[action_name]:
                print(f"{utils.state_to_str(hypotesis)}\n{'-'*40}")
            print(f"{'='*40}\n EFFECTS LOWER BOUND: {action_name}\n{'='*40}\n{utils.state_to_str(lower_effects[action_name])}")
            print(f"{'='*40}\n EFFECTS UPPER BOUND: {action_name}\n{'='*40}\n{utils.state_to_str(upper_effects[action_name])}")
    ...
    domain_exporter = PDDLExporter()
    domain_exporter.export_domain(sound_domain, sound_model_path)
