...
class AMLUtils:
    ...
    def state_to_str(self, state: State):
    ...
    def get_sorted_predicate_params(self, predicate: Predicate) -> List[str]:
    ...
    def get_sorted_action_params(self, action: Action) -> List[str]:
    ...
    def get_literals_space(self, domain: Domain, problem: Problem) -> State:
        """
        ...
        for predicate_name, predicate in domain.predicates.items():
            ...
            sorted_params = self.get_sorted_predicate_params(predicate)
    ...
    def check_type(self, obj_mapping: Dict[str, str], predicate: Predicate, action: Action) -> bool:
    ...
    def get_action_space(self, domain: Domain, action: str) -> State:
        """
        ...
        for predicate_name, predicate in domain.predicates.items():
            ...
            for action_params_combination in itertool_product(action_parameters, repeat=len(predicate.signature)):
                ...
                if self.check_type(obj_mapping, predicate, domain.actions[action]):
                    true_predicate = GroundedPredicate(predicate_name, predicate.signature, obj_mapping, True)
                    false_predicate = GroundedPredicate(predicate_name, predicate.signature, obj_mapping, False)
    ...
    def get_involved_parameters_substate(self, state: State, action_parameters: List[str]) -> Dict[str, Set[GroundedPredicate]]:
        """
    ...
    def get_parametrized_substate(
        ...
        # This sorted list is necessary for correct object mapping, using zip
        sorted_params = self.get_sorted_action_params(action)
    ...
    def is_hypotesis_subset(self, hypotesis: State, parametrized_state: Dict[str, Set[GroundedPredicate]]):
    ...
    def expand_hypotesis(self, hypotesis: State, parametrized_state: Dict[str, Set[GroundedPredicate]], action_space_lb: State) -> List[State]:
        result = []
    ...
    def get_parametrized_substate_fixed(self, substate: Dict[str, Set[GroundedPredicate]], action: Action, action_parameters: List[str], domain: Domain) -> Dict[str, Set[GroundedPredicate]]:
        """
        ...
        for predicate_name, predicates in substate.items():
            parametrized_substate[predicate_name] = set()
            predicate_signature = domain.predicates[predicate_name].signature
            sorted_predicate_params = self.get_sorted_predicate_params(domain.predicates[predicate_name])
    ...
    def update_preconds_ub(self, state: State, action: Action, action_parameters: List[str], action_space_lb: State, action_space_ub: List[State], domain: Domain):
        """
        ...
        """
        # Step 1
        substate: Dict[str, Set[GroundedPredicate]] = self.get_involved_parameters_substate(state, action_parameters)
        # Step 2
        parametrized_substate: Dict[str, Set[GroundedPredicate]] = self.get_parametrized_substate(substate, action, action_parameters, domain)
        ...
        for hypotesis in action_space_ub:
            ...
            if self.is_hypotesis_subset(hypotesis, parametrized_substate):
                expanded_hypotesis_set = self.expand_hypotesis(hypotesis, parametrized_substate, action_space_lb)
    ...
    def effect_union(self, state: State, next: State, action: Action, action_parameters: List[str], action_space_lb: State, domain: Domain) -> State:
        """
        ...
        parametrized_substate: Dict[str, Set[GroundedPredicate]] = self.get_parametrized_substate(state_grounded_effects, action, action_parameters, domain)
    ...
    def reduce_upper_bound(self, action_space_ub: List[State]):
        """
        ...
        for i in range(len(action_space_ub)):
            ...
            if not supersets[i]:
                ...
                for j in range(len(action_space_ub)):
                    ...
                    if not supersets[j] and i != j and self.is_hypotesis_subset(current, action_space_ub[j].state_predicates):
                        supersets[j] = True
    ...
    def copy_state(self, state: State):
        """Function that return the deep copy of a State"""
        if state.state_predicates is None:
            return None
    ...
    def get_effect_space(self, effect_space_lb: State, effect_space_ub: State) -> List[State]:
        """
        Generates all possible effect hypotheses for an action,
        constrained by the lower bound and the upper bound.

        Each hypothesis starts with the fixed effects from the
        lower bound and adds a non-empty combination of the difference
        between upper bound and lower bound.

        Args:
            effect_space_lb: The lower bound of effects.
            effect_space_ub: The upper bound of effects.

        Returns:
            A list where each State object represents an effect hypothesis.
        """
